...
app.get('/c/:id', (req, res) => {
    ...
    if (!findC) {
        res.sendStatus(DATA_1.HTTP_STATUS.NOT_FOUND_404);
        return;
    }
    res.json(findC);
});
app.post('/c', (req, res) => {
    ...
    res.status(DATA_1.HTTP_STATUS.OK_200);
    res.json(createCourse);
});
app.delete('/c/:id', (req, res) => {
    // fetch('http://localhost:3000/c/2', {method: 'DELETE'})
    db.courses = db.courses.filter(c => c.id !== +req.params.id);
    res.sendStatus(DATA_1.HTTP_STATUS.NO_CONTENT_204);
});
app.put('/c/:id', (req, res) => {
    if (!req.body.title) {
        res.sendStatus(DATA_1.HTTP_STATUS.BAD_REQ_400);
        return;
    }
